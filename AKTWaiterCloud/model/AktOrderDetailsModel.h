...
@property(nonatomic,strong) NSString <Optional>* maxLateTime; // 迟到最大时长     允许操作的最大时长（时长内只记录，时长外根据lateAbnormal判断）
...
@property(nonatomic,strong) NSString <Optional>* maxLocationDistanceSignIn; // 签入最大定位误差  前端对比：误差之内是正常
...
@property(nonatomic,strong) NSString <Optional>* soundRecordTimeSignIn; // 签入录音时长
...
@property(nonatomic,strong) NSString <Optional>* maxEarlyTime; // 早退最大时长
...
@property(nonatomic,strong) NSString <Optional>* minServiceLength; // 最低服务时长
