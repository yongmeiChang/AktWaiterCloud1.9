...
#define ASF_FACE_COMBINEDMASK ASF_FACE_DETECT | ASF_FACERECOGNITION | ASF_AGE | ASF_GENDER | ASF_FACE3DANGLE | ASF_LIVENESS
...
- (NSArray*)process:(ASF_CAMERA_DATA*)cameraData
{
    ...
    if(0 == dispatch_semaphore_wait(_processSemaphore, 0))
    {
        ...
        do {
            ...
            ASF_FaceLivenessScore score = {0};
            if(ASF_MOK != [self.arcsoftFace getLiveness:&score] || score.num != multiFaceInfo.faceNum)
                break;
            ...
            for (int face=0; face<multiFaceInfo.faceNum; face++) {
                ...
                faceInfo.liveness = score.scoreArray[face];
            }
        } while (NO);
