...
-(void)showImageIn{
    ...
    NSDictionary * param = @{@"workOrderId":self.orderId,@"tenantsId":appDelegate.userinfo.tenantsId,@"signType":@"101"};
...
-(void)showImageOut{
    [[AppDelegate sharedDelegate] showLoadingHUD:self.view msg:@"加载中..."];
    NSDictionary * param = @{@"workOrderId":self.orderId,@"tenantsId":appDelegate.userinfo.tenantsId,@"signType":@"102"};
    [[AFNetWorkingRequest sharedTool] requestgetWorkOrderImages:param type:HttpRequestTypePost success:^(id responseObject) {
        NSDictionary * dic = responseObject;
        NSNumber * code = dic[@"code"];
        if([code intValue]==1){
