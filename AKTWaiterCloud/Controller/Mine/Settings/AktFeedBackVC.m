...
- (IBAction)updateAllInfo:(UIButton *)sender {
    ...
    if (baseStr.length>1) {
        dicParam = @{@"mobile":tfPhone.text,@"tenantsId":kString([LoginModel gets].tenantId),@"waiterUkey":[UserInfo getsUser].uniqueKey,@"content":tvRemark.text,@"imageData":basearr,@"imageType":@"png"}; //
    }else{
