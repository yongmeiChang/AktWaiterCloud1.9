...
- (UIImage *)addWatemarkImage:(UIImage *)logoImage watemarkText:(NSString *)watemarkText {
    ...
    for (int i = 0; i < line; i ++) {
        for (int j = 0; j < row; j ++) {
            ...
            [[watemarkText substringToIndex:3] drawInRect:CGRectMake(j * (SCREEN_WIDTH/2.5), i*100, SCREEN_WIDTH/2.5, 18) withAttributes:dic]; // 水印字号 颜色 文字居中 第一行文字
        
            NSDictionary *dicSubtitle =@{  NSFontAttributeName:_textFontSubtitle,
                                   NSForegroundColorAttributeName:_textColor,
                                   NSParagraphStyleAttributeName:textStyle,
                                   NSObliquenessAttributeName:@-0.3
                                   };
            [[watemarkText substringWithRange:NSMakeRange(4,20)] drawInRect:CGRectMake(j * (SCREEN_WIDTH/2.5), i*100+18, SCREEN_WIDTH/2.5, 16) withAttributes:dicSubtitle]; // 第二行文字
